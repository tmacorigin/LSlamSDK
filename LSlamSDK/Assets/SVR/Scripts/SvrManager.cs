...
public class SvrManager : MonoBehaviour
{
	...
	void OnPreRenderListener (int textureId)
	{
        if (!IsRunning)
            return;
        ...
        plugin.SetFoveationParameters(textureId, FocalPoint.x, FocalPoint.y, currentGain.x, currentGain.y, currentArea);
    ...
    void OnFoveateChanged(Vector2 gain, float area)
    {
        ...
        var currentArea = area == 0f ?
            settings.foveationArea : SvrOverrideSettings.FoveateArea;
        plugin.SetFoveationParameters(0, point.x, point.y, currentGain.x, currentGain.y, currentArea);
    }
