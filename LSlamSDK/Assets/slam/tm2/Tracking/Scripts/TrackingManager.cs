...
//			device.writeAssociatedDevices (
//				new byte[]{ 0xde, 0xad, 0xbe, 0xef, 0x00, 0x00 },
//				new byte[]{ 0xde, 0xad, 0xbe, 0xef, 0x00, 0x01 });

			int i = 0;
			foreach (var d in device.readAssociatedDevices ())
				Debug.LogFormat ("Associated device #{0} = {1}", i++, String.Join (":", Array.ConvertAll (d, b => b.ToString ("X"))));
