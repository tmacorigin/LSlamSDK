...
//					arrivalTimeStamp = BitConverter.ToUInt64(b, 16),
			...
			void RaiseConnectionEvent (IntPtr controllerDevice, int index, IntPtr error)
			{
				var h = onConnect;
				if (h != null)
					h (index);
			...
			void RaiseDisconnectionEvent (IntPtr controllerDevice, int index)
			{
				var h = onDisconnect;
				if (h != null)
					h.Invoke (index);					
			...
			class ControllerListener : IDisposable
			{
				...
				public ControllerListener (Action<IntPtr> onControllerEvent, 
				                           Action<IntPtr, int, IntPtr> onConnectionEvent,
				                           Action<IntPtr, int> onDisconnectionEvent)
				{
					callbacks = new Callbacks {
						...
						onDisconnectionEvent = (IntPtr)GCHandle.Alloc (onDisconnectionEvent),
						onDisconnectionEventCallback = InvokeDisconnectionEvent,
				...
				[AOT.MonoPInvokeCallback (typeof(Action<IntPtr>))]
				static void InvokeConnectionEvent (IntPtr action, IntPtr ctrl, int index, IntPtr error)
				{
					(GCHandle.FromIntPtr (action).Target as Action<IntPtr,int,IntPtr>) (ctrl, index, error);
				}

				[AOT.MonoPInvokeCallback (typeof(Action<IntPtr>))]
				static void InvokeDisconnectionEvent (IntPtr action, IntPtr ctrl, int index)
				{
					(GCHandle.FromIntPtr (action).Target as Action<IntPtr, int>) (ctrl, index);
				}
				...
				public struct Callbacks
				{
					...
					public Action<IntPtr, IntPtr, int, IntPtr> onConnectionEventCallback;

					public IntPtr onDisconnectionEvent;

					public Action<IntPtr, IntPtr, int> onDisconnectionEventCallback;
				}
			}
			...
			public ControllerDevice (AndroidJavaObject instance)
			{
				...
				listener = new ControllerListener (RaiseControllerEvent, RaiseConnectionEvent, RaiseDisconnectionEvent);
//				GCHandle.Alloc (listener, GCHandleType.Pinned);
			...
			internal class ControllerInfo: IControllerInfo
			{
				readonly AndroidJavaObject m_info;

				internal ControllerInfo (AndroidJavaObject info)
				{
					m_info = info;
				}
				...
				public int VendorData {
					get {
						return m_info.Call<int> ("getVendorData");
					}
				}
			}
			...
			public IControllerInfo Info {
				get {
					return new ControllerInfo (instance.Call<AndroidJavaObject> ("getInfo"));
				}
			}
			...
			public int Connect ()
			{
				try {
					//!TODO: timeout
					int timeout = 2000;
					Id = (byte)instance.Call<int> ("connect", m_poseListener, timeout);
//					Index = instance.Call<int> ("connect", m_listener);
		...
		internal class TrackingDevice : ITrackingDevice
		{
			...
			public List<byte[]> readAssociatedDevices ()
			{
				...
				var l = new List<byte[]> ();
				using (var list = m_dev.Call<AndroidJavaObject> ("readAssociatedDevices")) {

					using (var a = list.Call <AndroidJavaObject> ("get", 0))
						l.Add (AndroidJNIHelper.ConvertFromJNIArray<byte[]> (a.GetRawObject ()));

					using (var a = list.Call <AndroidJavaObject> ("get", 1))
						l.Add (AndroidJNIHelper.ConvertFromJNIArray<byte[]> (a.GetRawObject ()));
				}
				return l;
			}
			...
			{
				m_dev.Call ("writeAssociatedDevices", mac1, mac2);
			}
...
//						throw new Exception (e.ToString ());
		...
		internal static TrackingDevice getTrackingDevice ()
		{
			using (var javaUnityPlayer = new AndroidJavaClass ("com.unity3d.player.UnityPlayer")) {
				using (var currentActivity = javaUnityPlayer.GetStatic<AndroidJavaObject> ("currentActivity")) {
					using (var tm_class = new AndroidJavaClass ("com.intel.realsense.tracking.TrackingManager")) {
						AndroidJavaObject dev = tm_class.CallStatic<AndroidJavaObject> ("getTrackingDevice", currentActivity, null);
						if (dev == null) {
